...
- When the user asks only for Plan work, produce strategy, sequencing, dependency analysis, risk handling, validation strategy, rollout guidance, or task candidates without changing source code.
- It is acceptable to reference code paths, describe implementation options, or include clearly labeled illustrative pseudocode when needed for planning clarity, but do not apply code changes without explicit user confirmation.
